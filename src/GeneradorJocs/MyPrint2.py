...
def printInit(numHab,numRes,numDies, rng,file = original_stdout):
    ...
    #orientacio
    for h in range(numHab):
        printOrientacioHabitacio(h, rng.getOrientation(),file)
...
def printStart(file = original_stdout):
    print("(define (problem ext2) (:domain reserves)", file = file)
    return
...
def printOrientacioHabitacio(hab, num,file = original_stdout):
    print("    (= (orientacio_habitacio habitacio" + str(hab) + ") " +  str(num) + ")", file = file)
    return
...
def printMalOrientadesReserva( file = original_stdout):
    print("    (= (reserves_mal_orientades) 0)", file = file)
    return
